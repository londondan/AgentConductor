...
2. Resolve `${CLAUDE_PLUGIN_ROOT}` to its actual filesystem path (so the printed command works in a plain shell), then print this command for the user to copy-paste into a NEW terminal window:
```bash
python3 <RESOLVED_PLUGIN_ROOT>/scripts/monitor.py --session-id <SESSION_ID>
```
